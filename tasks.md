...
- [~] **SHIP-21** `AUTO` — Brand invariants. **Emoji DONE**: 21a (`c1eab2c`) removed the one prose-copy emoji (✅→SVG); 21b (`496df2a`) replaced all 142 emoji-as-iconography with an inline-SVG icon set (`apps/web/components/icons/index.tsx`) across 19 files — emoji scan 142→0, typecheck/lint/build green, home+how-it-works screenshot-verified light/dark. Kimal chose "full SVG icon set." **No numeric subscriber count** renders anywhere (invariant trivially met). **REMAINING (greenfield builds):** `SubscriberCount` component (hide <2,500 → qualitative copy per §8) + `SponsorBlock` component (locked labels "Sponsored by"/"Partner message from" + `data-firewall=32` from wordmark; refactor the HeroCarousel `type:"sponsor"` slide to use it). _deps:_ SHIP-08
