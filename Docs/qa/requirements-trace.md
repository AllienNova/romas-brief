...
| FR-008 | -16 LUFS / -1 dBTP, -17..-15 enforced at QA gate (L55) | **TRACED** | T-205 (10-beat script, MASTER L129) — product-spec cell wrong (should be T-204 loudness), T-208 — also wrong (should be T-204 + T-209 + T-220). | Catalog: **A-015** loudness out of band (test-qa L131), **A-016** true_peak too hot (L132), **A-024** loudness fixture WAV in [-17,-15] (L145), **A-025** loudness rejection on hot fixture (L146). | **P2 doc.** Product-spec maps FR-008 to "T-205, T-208" — both wrong. Should be T-204 + T-209 + T-220 (retry cap). |
| FR-009 | **Five-condition** QA gate flip: `clinical_claims_checked=true` AND `qa_reviewer NOT NULL` AND `loudness_lufs BETWEEN -17 AND -15` AND `true_peak_dbtp <= -1` AND `transcript_url NOT NULL` (L56, cites `contracts/supabase-schema.sql:123-130`) | **TRACED** | T-104 (audio_jobs CHECK, MASTER L89), T-209 (QA UI enforces all 5, MASTER L133, delivery L108). | Catalog covers **all 5 conditions**: **A-013** qa_reviewer null (test-qa L129), **A-014** clinical_claims_checked false (L130), **A-015** loudness OOB (L131), **A-016** true_peak too hot (L132), **A-017** transcript_url null (L133). Plus **A-030** "all 4 conditions absent" (L158). | **Cycle-2 fix verified.** Critic F-P1-01 (5-vs-4 drift) cleanly resolved: product-spec FR-009 now lists all 5 conditions including `true_peak_dbtp <= -1`. **One residual P2:** A-030 title still says "blocks all 4 conditions absent" (test-qa L158) — should be "all 5". Cosmetic. |
