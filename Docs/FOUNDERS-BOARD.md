...
- **New provisioning P-items (Kimal):**
  - **P-25 · Provision the OpenClaw agent layer** — code is BUILT + shipped (`@romas-brief/agent-tools` + `infra/openclaw/`, ADR-0020 Accepted); only runtime provisioning remains. **Runtime = NVIDIA NemoClaw / OpenShell** hosting OpenClaw (ADR-0020 Amendment A1 — the hardened runtime gives us OS isolation/egress/containment natively; zero rework to our MCP tools). Kimal provides: a **Linux host** for NemoClaw/OpenShell; `VERCEL_AI_GATEWAY_URL`/`_KEY`; `RESEND_API_KEY`; `TWILIO_ACCOUNT_SID`/`AUTH_TOKEN`/`FROM`; `BEEHIIV_API_KEY`/`PUBLICATION_ID`; read-only `SUPABASE_ANON_KEY`; `ROMAS_APPROVAL_TOKEN`. **Verify-before-prod (rule 11):** NemoClaw runs our custom `openclaw.json`+MCP server; OpenShell allows an OpenAI-compatible `base_url` for the Vercel Gateway; Linux Landlock/seccomp host reqs; supply-chain audit. See `infra/openclaw/README.md`.
