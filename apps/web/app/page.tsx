...
function SectionHeader({
  ...
  return (
    <div className="flex items-end justify-between mb-8 gap-4">
      <div>
        <p className="kicker mb-1.5" style={{ color: "var(--rb-text-tertiary)" }}>{label}</p>
        <h2
        ...
        {description && (
          <p className="mt-1.5 text-sm" style={{ color: "var(--rb-text-secondary)" }}>{description}</p>
        )}
      </div>
      {href && (
        <Link
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          <div className="lg:col-span-5">
            ...
            <div className="rounded-2xl border overflow-hidden" style={{ background: "var(--rb-bg-surface)", borderColor: "var(--rb-border-subtle)", boxShadow: "var(--rb-shadow-sm)" }}>
                  ...
                  <div className={`p-5 transition-colors hover:bg-[var(--rb-bg-raised)] ${i < industryMoves.length - 1 ? "border-b" : ""}`} style={{ borderColor: "var(--rb-border-subtle)" }}>
                    <div className="flex items-start gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <time className="kicker mt-1 block" dateTime={article.published_at} style={{ color: "var(--rb-text-tertiary)" }}>
                    ...
                    <div className="grid grid-cols-3 gap-3 p-4 rounded-xl mb-6" style={{ background: "var(--rb-bg-raised)" }}>
                      {[
                        ...
                        <div key={label} className="text-center">
                          ...
                          <p className="kicker mt-0.5" style={{ color: "var(--rb-text-tertiary)" }}>{label}</p>
                        </div>
                      ))}
                    </div>
                    {paperOfDay.romas_insight && (
                      <div className="p-4 rounded-xl border-l-4 mb-5" style={{ background: "var(--rb-accent-subtle)", borderLeftColor: "var(--rb-accent)" }}>
                        <p className="kicker mb-1" style={{ color: "var(--rb-accent)" }}>ROMAS Insight</p>
                        <p className="text-sm leading-relaxed" style={{ color: "var(--rb-text-primary)" }}>{paperOfDay.romas_insight}</p>
                      </div>
                    )}
              ...
              <div className="rounded-2xl overflow-hidden" style={{ background: "linear-gradient(135deg, #001D3D 0%, #001A18 100%)", boxShadow: "var(--rb-shadow-lg)" }}>
                <div className="p-8">
                  ...
                  <div className="flex items-center gap-4 p-4 rounded-xl" style={{ background: "rgba(255,255,255,0.08)" }}>
                    <button className="w-12 h-12 rounded-full flex items-center justify-center transition-all hover:scale-105 active:scale-95" style={{ background: "var(--rb-accent)" }} aria-label="Play episode">
                      <svg className="w-5 h-5 text-white ml-0.5" fill="currentColor" viewBox="0 0 24 24"><path d="M8 5v14l11-7z" /></svg>
                    </button>
                    <div className="flex-1">
                      <div className="h-1.5 rounded-full overflow-hidden" style={{ background: "rgba(255,255,255,0.15)" }}>
                        <div className="h-full rounded-full" style={{ width: "0%", background: "var(--rb-accent)" }} />
                      </div>
                  ...
                  <div className="flex items-center gap-3 mt-4">
                    ...
                    <Link href="/listen" className="flex-1 text-center py-2.5 rounded-xl text-sm font-semibold hover:opacity-90" style={{ background: "var(--rb-accent)", color: "white" }} aria-label="Browse all audio">Browse all audio</Link>
                  </div>
              ...
              <div className="space-y-0">
                {trending.map((article, i) => (
                  <Link key={article.slug} href={`/article/${article.slug}`} className="block group" aria-label={`Trending #${i + 1}: ${article.title}`}>
                    <div className={`flex items-start gap-5 py-5 transition-colors hover:bg-[var(--rb-bg-surface)] rounded-xl px-3 -mx-3 ${i < trending.length - 1 ? "border-b" : ""}`} style={{ borderColor: "var(--rb-border-subtle)" }}>
                      <span className="flex-shrink-0 text-4xl font-black tabular-nums" style={{ color: "var(--rb-border-strong)", lineHeight: "1" }}>{i + 1}</span>
                      <div className="flex-1 min-w-0">
          ...
          {topPapers.map((article) => (
            <Link key={article.slug} href={`/article/${article.slug}`} className="block group" aria-label={`Research paper: ${article.title}`}>
              <article className="card h-full p-5">
                ...
                {article.romas_insight && <p className="text-xs line-clamp-2 mb-3 leading-relaxed" style={{ color: "var(--rb-text-secondary)" }}>{article.romas_insight}</p>}
                <div className="mt-auto pt-3 border-t flex items-center justify-between" style={{ borderColor: "var(--rb-border-subtle)" }}>
                  <time className="kicker" dateTime={article.published_at} style={{ color: "var(--rb-text-tertiary)" }}>
                    {new Date(article.published_at).toLocaleDateString("en-US", { month: "short", day: "numeric" })}
                  </time>
