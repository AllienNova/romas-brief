...
- **New provisioning P-items (Kimal):**
  - **P-25 · Provision the OpenClaw agent layer** — code is BUILT + shipped (`@romas-brief/agent-tools` + `infra/openclaw/`, ADR-0020 Accepted); only runtime provisioning remains. **Runtime = NVIDIA NemoClaw / OpenShell** hosting OpenClaw (ADR-0020 Amendment A1 — the hardened runtime gives us OS isolation/egress/containment natively; zero rework to our MCP tools). Kimal provides: a **host** (Linux+Docker preferred / macOS Apple-Silicon / WSL2 — **Node 22.16+**, Docker, **no GPU needed**, ≥4 vCPU/8 GB/20 GB; Linux kernel ≥5.13 for Landlock); `VERCEL_AI_GATEWAY_URL`/`_KEY`; `RESEND_API_KEY`; `TWILIO_ACCOUNT_SID`/`AUTH_TOKEN`/`FROM`; `BEEHIIV_API_KEY`/`PUBLICATION_ID`; read-only `SUPABASE_ANON_KEY`; `ROMAS_APPROVAL_TOKEN`. **Verification (P-25-spike, 2026-06-02): mostly CLEARED** — NemoClaw runs our custom config via a baked sandbox image (`nemoclaw onboard --from Dockerfile`); the Vercel Gateway onboards as a `compatible-endpoint`; isolation = Landlock + cap-drop + egress policy. **Remaining build-time:** author the ROMAS `infra/openclaw/Dockerfile` (needs NemoClaw base-image name + extensions path), confirm MCP server spawns in-sandbox, supply-chain audit + pin. See `infra/openclaw/README.md`.
