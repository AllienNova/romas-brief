...
| R-105 | G-003 | Add pgTAP schema-constraint tests for `audio_publish_requires_qa`, `articles_primary_source_required` (includes ADR-0016 URL scheme regex), `articles_embargo_consistency`, `articles_insight_labeled`, `articles_published_requires_author` (build-2026-05-21 A9), `audio_revoke_requires_reason`, `audio_skip_requires_reason`, `embargo_release_pair` (build-2026-05-21 A2), `audio_jobs_article_tier_uniq` (build-2026-05-21 A6 + ADR-0017), `qa_reviewers.email` lower-case CHECK (build-2026-05-21 A10), `articles.body_md` length cap (build-2026-05-21 A4), `audio_jobs.script_md` length cap (build-2026-05-21 A4), `claims.source_url` HTTPS scheme regex (build-2026-05-21 A3), `sources.feed_url` + `sources.api_endpoint` HTTPS scheme regex (build-2026-05-21 A3). Also Cycle-1 P2-05 carry: archetype/tier/status enums, claims.confidence, qa_reviewers.role, audio_jobs.audio_tier, title length. | cms-engineer | M | A-114..A-119, plus new A-NNN to be allocated for the build-2026-05-21 additions |
...
| R-202 | G-016 | Implement two-pass ffmpeg `loudnorm` measurement; **write `loudness_lufs` + `true_peak_dbtp` to `audio_jobs` table**. Pipeline targets -16 ±0.5 LUFS; tolerate -16 ±1 LUFS (`[-17, -15]`) without re-master; outside `[-17, -15]` re-master once with adjusted `I=-16 TP=-1 LRA=11`; if still outside `[-18, -14]` (ADR-0016 DB gate) mark job `skipped` with reason `loudness_out_of_band`. TP > -1 dBTP always re-masters. | audio-producer | M | A-025, A-216 |
