...
- [x] **SHIP-12** `AUTO*` (live send needs P-06/P-14) — DONE (commit 2c56772): Resend transactional+webhook worker in `email-canary` (dir kept; rename cosmetic-deferred). Svix verify (whsec_ HMAC-SHA256, 300s tolerance, constant-time), 4 templates, send via fetch + Idempotency-Key, delivery-event→subscriber bounce/complaint map. 35 node:test; typecheck/build 0; zero new deps. _Follow-on:_ live needs RESEND_API_KEY/RESEND_WEBHOOK_SECRET/INTERNAL_SEND_SECRET + Resend DNS (P-14). _deps:_ SHIP-06
- [x] **SHIP-13** `AUTO` — DONE (satisfied by SHIP-08 + rss-publisher; no new code): homepage Today's-Podcast (module 5), Trending, Top-Papers sections are wired to `@/lib/articles` (real data via `Promise.all`, page.tsx:185-192) — verified by the green web build; `daily-brief.xml` (Tier-2 roundup) already generated by `workers/rss-publisher` (dbTier `daily_brief`). _Caveat:_ Today's-Podcast/audio modules render empty against the live DB until the `audio_jobs` join lands (SHIP-23) + audio content exists (P-21). _deps:_ SHIP-08, SHIP-09

## WAVE 3 — HARDEN (correctness FIRST, then tests on corrected code)

- [x] **SHIP-14** `AUTO` — DONE (commit 0dbf373, PR #8): `LOUDNORM_ENDPOINT` in `Env` + **fail-closed** (no endpoint → skip); removed RMS `loudnormInline` + **dead PlayHT** (ADR-0018/19); failover = ElevenLabs retry 3× (2s/8s/30s)→`skipped`; model-id const; pure fns + 6 node:test. typecheck/build 0; `grep playhtTts/PLAYHT_/loudnormInline(`=0. _Deferred:_ cross-vendor failover seam pending **Q-F**. _deps:_ SHIP-08
- [x] **SHIP-15** `AUTO` — DONE (commit 78dcaae): RSS enclosure length = `estimateEnclosureBytes(duration_sec)` (was "0"); `/regenerate` (missing→all/valid→one/invalid→400); `CDN_BASE_URL` env (dropped hardcoded, throws if unset; real domain pending P-12); audio-producer embargo-gate (Phase 0c → skip `article_embargoed`); NMPA/M-02 satisfied-by-absence (no NMPA source; guard documented). 16 node:test; lint/typecheck/build/audit 0. _deps:_ SHIP-08
- [x] **SHIP-16** `AUTO` — DONE (commit cb8b9d4): migration 0012 `WITH CHECK` on `editor_publish`+`audio_qa_flip` + `articles_published_requires_tags` (gate #8: audience+region; modality editorial-enforced) + documented DOWN; pgTAP (with_check_and_tags.sql); `AbortSignal.timeout(10000)` on reader query chokepoint (M-01). Validated UP+DOWN via rollback txn (DB unchanged); lint/typecheck/build/audit 0. **0012 not applied to live DB (gated)** — applies via deploy-migrations pipeline. _Deferred (documented):_ broad cms/worker AbortSignal + cross-edition revoke re-check (no issue-publisher dispatcher yet). _deps:_ SHIP-06
- [~] **SHIP-17** `AUTO` — Test pyramid backfill. **17a DONE** (PR #11, squashed `f96b8b7`): wired CI `test` to real `node:test` suites — web markdown-XSS ×8, cms audio-QA Rule-6 gate ×5 (new `audio-qa-gate.test.ts`), shared signal-scoring ×8; plus pre-existing worker suites. `turbo run test` 14/14 green; typecheck+lint green. **Discovered + diagnosed R-C7-8**: GitHub Actions has *never* run (Free-org+private-repo billing block, not code) → "CI `test` green" cannot be observed until P-00 (Kimal billing). **17b BLOCKED**: integration tests (CMS QA gate / Beehiiv / Resend / revocation race) need a real test DB. `BLOCKED: no Supabase test project | unblocker: P-00 Actions billing + a dedicated test project URL/key (or local supabase stack) | next: author skip-when-no-creds integration harness once test DB exists`. _deps:_ SHIP-09..16
...
- [x] **SHIP-20** `AUTO` — **DONE** (20a `8e52830` + rename `1bb91d6`). Added the spec-mandated `--rb-audio-{published,pending,skipped}` tokens (light+dark) to `globals.css`. Renamed the entire implemented token set to the canonical `--rb-*` prefix (`--accent/--bg-*/--text-*/--border-*/--teal/--shadow-*/--ease-*/--radius-*`) across 21 source files (globals.css + 5 route/layout + 15 components) — value-preserving (per Kimal's "full rename" choice). Tailwind color names left alone (not CSS vars). **Verified** (next start + Playwright): 0 orphaned/unprefixed, 0 double-prefix; web build green; light body bg `#F8F8FA` ← `--rb-bg-page`, dark body bg `#000000` + full dark theme renders correctly (screenshot). Dark-mode route audit: of 35 flagged light-only classes, 8 already paired with `dark:` variants, rest are intentional white-on-color hero decoration/buttons → **no systemic breakage**. _Minor follow-ups (not SHIP-20):_ `--rb-shadow-card` is referenced (academy ×3, FromTheEditor) but undefined (pre-existing, renders no shadow); React #418 hydration warning on home (pre-existing, unrelated to CSS). _deps:_ SHIP-08
- [x] **SHIP-21** `AUTO` — **DONE** (21a `c1eab2c` · 21b `496df2a` · finish `eb55f50`). **Emoji**: removed the prose-copy ✅; replaced all 142 emoji-as-iconography with an inline-SVG set (`components/icons/index.tsx`) across 19 files — scan 142→0, screenshot-verified light/dark. **SubscriberCount**: new component (hide <2,500 → qualitative §8 line), wired LIVE into the layout footer (device-testing caught a first attempt landing in dead `SiteFooter`); also removed 5 fabricated "4,200+" count claims (integrity fix — we have ~0 subs). **SponsorBlock**: new component with the 2 locked labels + 32px firewall (`data-firewall=32`); `SPONSOR_FIREWALL_PX` applied live to the carousel sponsor slide. Verify: footer renders qualitative line, 0 "4,200", typecheck/lint/build green. _Note:_ SponsorBlock card body-mount + SiteHeader/SiteFooter dead-code deletion → SHIP-23. _deps:_ SHIP-08
- [~] **SHIP-22** `AUTO` — A11y WCAG 2.2 AA. **DONE** (22a `5896eaf` · 22b `018b604`): skip-to-content link (2.4.1); single visually-hidden homepage `<h1>` (1.3.1); reduced-motion guard on RotatingTopStories auto-advance + progress (2.3.3); scrubber `aria-valuetext` (4.1.2, native keyboard ok); icons aria-hidden-when-decorative (icon module default); `--rb-text-tertiary` darkened for contrast (light #AEAEB2→#6E6E73, dark→#7C7C82); pagination dots → 24px hit targets (2.5.8). **axe-core verified: 0 critical both themes; light color-contrast 84→15; target-size 13→0.** **REMAINDER (axe serious, documented):** light 15 = signal/audio badge fg/bg pairs + 1 `#d1d1d1` UI element; **dark 113 = hardcoded Tailwind gray classes (`text-neutral-600/900`) that don't adapt to dark mode** — a `--rb-*` token-migration sweep (residue of SHIP-20's dark audit, concentrated in the article page). Lighthouse a11y ≥95 pending that sweep. _deps:_ SHIP-20
- [x] **SHIP-23** `AUTO` — **DONE** (23a `c030219` · 23b `2ac4ce7`). `AudioStatusBadge` (6-state, maps the migration-0002 audio_status enum → 3 `--rb-audio-*` tokens; status model in `lib/audio-status.ts`, 4 unit tests). `AudioPlayer` (Variant A inline = reuses `InlineAudioPlayer`; Variant B banner "Listen to this brief"); **QA-gated via `isAudioPlayable`** (Rule 6 — only `published`+CDN-URL plays, else renders the badge). Wired into the article page (banner, real `audio_brief` tier + duration); replaced related-article `▶` glyph with `AudioStatusBadge`. **Deleted dead `SiteHeader`/`SiteFooter`** (imported nowhere; layout.tsx owns header+footer). Verify: header "Audio available" + 3 related `AudioStatusBadge` (aria-label "Audio status: Audio") + 0 ▶ on `/article/flash-rt-…`; typecheck/lint/build green; web tests 12/12. _Note:_ banner player path needs a real `audio_url` (mock has none — same gate as before, no regression); Variant A ready for the listen-list slot. _deps:_ SHIP-10, SHIP-20
