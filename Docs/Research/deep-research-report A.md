...
```mermaid
flowchart TB
    %% Overall pipeline for ROMAS Wire
    subgraph Ingestion
...
- **Podcast Script:** Use conversational tone. For example, “Good morning – here’s your ROMAS Wire for today. In our top story, *X happened*, which means *Y*. Coming up: latest AI update and research highlight.” Write in short sentences, with natural fillers for speech. End with “thanks for listening, and subscribe”.  
...
```mermaid
flowchart LR
    A[Header: ROMAS Wire Logo + Subscribe Button] 
...
Over the long term, ROMAS Wire will evolve through these phases:
