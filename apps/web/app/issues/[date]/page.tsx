    ...
    <article className="group py-5 border-b border-[var(--rb-border-subtle)] last:border-0">
      <Link href={`/article/${article.slug}`} className="block">
        <div className="flex flex-wrap items-center gap-2 mb-2">
          ...
          <span className="ml-auto text-xs font-bold tabular-nums text-[var(--rb-text-tertiary)]">Score: {article.composite_score}</span>
        </div>
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 sm:px-6 py-12">
      ...
      <nav className="flex items-center gap-2 text-xs text-[var(--rb-text-tertiary)] mb-8">
        ...
        <span>/</span>
        <Link href="/issues" className="hover:text-[var(--rb-text-secondary)] transition-colors">Archive</Link>
        <span>/</span>
        <span className="text-[var(--rb-text-secondary)]">{params.date}</span>
      </nav>

      {/* Header */}
      <div className="mb-8">
        <p className="text-xs uppercase tracking-widest font-semibold text-teal-600 mb-2">Daily Issue</p>
        <h1 className="text-3xl font-black text-[var(--rb-text-primary)] tracking-tight">{label}</h1>
        <p className="mt-2 text-xs text-[var(--rb-text-tertiary)]">{articles.length} briefings in this issue</p>
      </div>
      ...
      <div className="mt-10 flex items-center justify-between pt-6 border-t border-[var(--rb-border-subtle)]">
        {prevDate ? (
          <Link
            ...
            className="flex items-center gap-2 text-sm text-[var(--rb-text-secondary)] hover:text-teal-700 transition-colors"
          >
            ← {new Date(prevDate + "T12:00:00Z").toLocaleDateString("en-US", { month: "short", day: "numeric" })}
          </Link>
        ) : <span />}
        <Link href="/issues" className="text-xs text-[var(--rb-text-tertiary)] hover:text-[var(--rb-text-secondary)] transition-colors">
          All issues
        </Link>
        {nextDate ? (
          <Link
